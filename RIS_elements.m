...
phase_elements = 20:20:100;

opt_power = zeros(1,length(phase_elements));

for ii = 1:length(phase_elements)
...
%                 ,W,H_rk,H_bk,U,w_k,Q_bar,K_I,K_E,M,alpha_l,eta,phi);
    ...
    power_ele = power_sim(:,end);
    
    opt_power(ii) = mean(power_ele);
...
hold on
plot(phase_elements,smooth(opt_power),'DisplayName',strcat('M=',num2str(M)));
